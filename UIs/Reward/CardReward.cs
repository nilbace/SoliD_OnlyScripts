...
public class CardReward : MonoBehaviour
{
    ...
    void SetUpCardRewards()
    {
        ...
        if (Random.value <= RareCardProbability)
        {
            ...
            var rareCard = GameManager.Card_RelicContainer.GetRandomCardDatas(1, E_CardTier.Rare);
            var normalCards = GameManager.Card_RelicContainer.GetRandomCardDatas(2, E_CardTier.Normal);
        ...
        {
            // 일반 카드 3장 선택
            var normalCards = GameManager.Card_RelicContainer.GetRandomCardDatas(3, E_CardTier.Normal);
            _rewardCards.AddRange(normalCards);
        }
